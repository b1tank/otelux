...
oteluxctl agents inspect claude-code [--json]
oteluxctl agents show-config claude-code [--json]
```
...
Read-only `agents` commands use `@otelux/agent-integrations` and do not require the runtime. The first adapter reports bounded Claude Code detection, MCP/plugin/skills capability state, and inspected path metadata without returning command output or config values. Mutation commands are not implemented yet.
...
Exit codes: `0` success/healthy, `1` invalid or internal failure, `2` runtime not running, `3` incompatible runtime, `4` runtime reachable with listener/settings issues, `5` settings revision conflict, `6` requested agent not detected.
