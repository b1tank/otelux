...
Download `SHA256SUMS` and one package from the [v0.1.17 GitHub Release](https://github.com/b1tank/otelux/releases/tag/v0.1.17).
...
For Linux arm64, use `OTelux-0.1.17-arm64.deb` or `OTelux-0.1.17-arm64.AppImage` and the matching checksum line.

| Platform | Package | Status |
| --- | --- | --- |
| Linux x64 | `.deb`, AppImage | Qualified prerelease |
| Linux arm64 | `.deb`, AppImage | Qualified prerelease |
| macOS arm64/x64 | DMG, ZIP | Unsigned preview; signing and notarization planned |
...
| CLI | Run OTelux headlessly, inspect health and endpoints, manage settings, and open the browser workbench. | Private bundled `oteluxctl` lifecycle/status/config/doctor client live on `main`; browser open, SQLite quick-check, and public PATH distribution remain. |
