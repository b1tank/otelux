...
For Linux arm64, use `OTelux-0.1.11-arm64.deb` or `OTelux-0.1.11-arm64.AppImage` and the matching checksum line. `0.1.12` exists only as an unpublished local candidate while hosted Actions is unavailable.
...
| CLI | Run OTelux headlessly, inspect health and endpoints, manage settings, and open the browser workbench. | Private bundled `oteluxctl` lifecycle/status/config client live on `main`; expanded doctor, browser open, and public PATH distribution remain. |
