...
Node hosts can use `connectRuntimeClient()` to discover and authenticate the owner published in `runtime.json`, or `ensureRuntimeClient()` to invoke one host-supplied start action and wait through bounded publication races. Discovery validates the canonical owner-only `runtime-token`, Runtime RPC negotiation, and live instance identity before returning the shared HTTP/SSE client; it never opens SQLite or trusts a token path redirected by state.
