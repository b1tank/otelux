...
An optional `onSqlExecute` observer supports statement-budget diagnostics without affecting normal storage instances. The test suite enforces fixed statement counts, one transaction per ingest/clear operation, and representative `EXPLAIN QUERY PLAN` index use for all current query families.
