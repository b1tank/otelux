...
| **Search is one shared input pattern** | Every surface uses the same `SearchField` interaction and clear affordance. Trace search covers trace/span IDs, names, and span attribute keys/values; logs cover IDs/body/attributes/resource/scope; metrics cover identity/metadata/resource/scope/point attributes. Detail search filters visible key/value content and opens matching sections. |
