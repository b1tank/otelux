...
- Historical `v0.1.0-beta.1`/`v0.1.0` launch naming is superseded: cross-platform prereleases have advanced through published `v0.1.14`, while the supported shared-runtime/agent milestone targets `v0.2.0` only after its acceptance gates pass.
...
- Every desktop version bump, including patch releases, must produce a matching `v<version>` tag and GitHub Release from the same commit. `v0.1.14` is a published unsigned cross-platform prerelease; macOS signing/notarization and Windows support remain release gates.
