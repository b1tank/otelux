...
- Historical `v0.1.0-beta.1`/`v0.1.0` launch naming is superseded: cross-platform prereleases have advanced through published `v0.1.15`, while the supported shared-runtime/agent milestone targets `v0.2.0` only after its acceptance gates pass.
...
- Every desktop version bump, including patch releases, must produce a matching `v<version>` tag and GitHub Release from the same commit. `v0.1.15` is a published unsigned cross-platform prerelease; macOS signing/notarization and Windows support remain release gates.
...
|---|---|---|
| 2026-08-26 | Public source restored | Reopened the repository as an explicit Linux prerelease after current-tree and history-pattern review, fresh-worktree lint/typecheck/test/build, a zero production audit, public documentation reconciliation, and enabling private vulnerability reporting, secret scanning, push protection, and Dependabot security updates. Public CI and CodeQL verification run on this commit. |
...
| 2026-07-14 | Settings-write rollback | Extracted the two-phase settings update behind small controller interfaces so it is testable without Electron or real ports, and closed the gap where a failed `settings.json` write left listeners rebound to the new ports while disk kept the old values. All mutated listeners now roll back to their previous shape on commit failure (and on receiver/MCP bind failure), so the running state always matches persisted settings. The implementation and tests now live under `packages/local-runtime`. Full lint/typecheck/test/build green. |
| 2026-07-14 | MCP per-install credential | Chose a per-install bearer token over disabling MCP so the feature stays usable out of the box. Added optional `HttpRouterOptions.authToken` to `@otelux/mcp-server`: when set, every JSON-RPC `POST` must send `Authorization: Bearer <token>` (constant-time compared via `timingSafeEqual`) or receive `401` before any tool runs; the identity probe stays open. The runtime generates a 32-byte URL-safe token on first run, stores it as `mcp-token` in the canonical data directory with owner-only permissions, reuses it across restarts, and logs the token-file path on startup. Updated the security model, privacy, spec, and manual test plan. MCP package 14 → 18 tests; full lint/typecheck/test/build green. |
...
| 2026-07-14 | Packaging hygiene | Tightened the electron-builder `files` allowlist so artifacts ship only built runtime code. A `--linux dir` pack revealed the symlinked `@otelux/*` workspace deps were shipping their entire `src/` (including `.test.ts`) and `dist/*.map`; added excludes for all sourcemaps and the workspace packages' `src/`, `.turbo`, tsconfig, tsup, and vitest config. Re-packing confirms zero `.map` files, zero workspace source/tests/cache logs, and the runtime `dist/` retained (every `@otelux` package resolves its entry to `dist`). Smoke-launched the packed binary: `/healthz` returns 200, OTLP and MCP listeners bind, and the per-install MCP token is generated. |
| 2026-07-16 | Canonical runtime ownership | Added platform data-home resolution (`XDG_DATA_HOME`, Application Support, or LocalAppData), `OTELUX_DATA_DIR` isolation, nonce-protected `runtime.lock`, atomically published `runtime.json`, stale-PID reclamation, and resumable copy-only migration from legacy Electron data. Custom database paths remain external; two populated default databases are preserved as a conflict rather than merged. Desktop passes its old user-data directory as migration input and the plugin bridge discovers canonical state/token first. Unit/integration coverage is 25 runtime tests. A two-launch live smoke ingested a trace into a legacy SQLite store, migrated to an empty canonical home, queried the exact trace over authenticated MCP, verified effective port state, and confirmed all legacy source files remained. The packaged smoke now verifies state/lock publication and clean signal-driven removal. |
| 2026-07-16 | Composite span identity | Fixed the storage audit P0 where `span_id` was treated as globally unique even though OTLP scopes it to a trace. Protocol `0.2.0`, storage, engine, Desktop IPC, MCP, and skills now require `(traceId, spanId)`. SQLite schema v2 transactionally rebuilds the span table with a composite primary key, recreates trace/parent indexes, repairs surviving materialized trace rollups, and deletes ghost summaries that no longer have spans. Failed forward migrations now preserve schema v1 at the canonical path for retry instead of activating an empty recovery database. The shared memory/SQLite contract tests equal span IDs in different traces, the on-disk v1 fixtures verify migration/data/rollup repair and failed-upgrade retry, and the original corruption probe now returns two traces with one correct span each. Plugin manifests advance to `0.1.5`. |
| 2026-07-16 | Packaged preload blank-screen fix | Fixed the `0.1.0` Linux package opening to a blank workbench because `shared/ipc.ts` caused the sandboxed preload bundle to emit `require("@otelux/protocol")`, which Electron's sandbox loader cannot resolve. Protocol constants now import directly in main/renderer, leaving the preload bundle dependent only on Electron. Every desktop build rejects unsupported preload `require()` calls, and the packaged smoke now asserts `window.otelux` exists plus the Traces workbench rendered before testing ingest. The unpacked package passed the strengthened smoke and a deskpal visual check. Desktop advances to `0.1.1`. |
| 2026-07-16 | Desktop tray lifecycle | Closing the desktop window now hides it to a packaged status-notifier tray instead of stopping the embedded runtime. Tray click or **Open OTelux** restores the workbench; **Quit OTelux**, SIGTERM, and OS quit share one explicit shutdown path that stops OTLP/MCP, closes SQLite, removes runtime ownership files, and unregisters the tray. Added a pure lifecycle controller with six tests, packaged dedicated app/tray icons, and extended the package smoke to prove ingest remains healthy after renderer close. A real GNOME session verified Alt+F4, registered menu labels, tray restore, full tray quit, listener refusal, and ownership cleanup. |
| 2026-07-17 | SQLite retention budget meter | Added protocol `0.3.0` storage-usage snapshots and a live battery-style meter under Settings → Data retention. The fill uses `page_count × page_size`, exactly matching size-pruning decisions; a separate DB/WAL/SHM breakdown reports physical disk footprint. Settings refreshes on telemetry/runtime events and every two seconds while open, previews edited age/size limits, and handles unlimited limits without fake percentages. Added engine-node, runtime, and component regressions. Desktop advances to `0.1.2`. |
| 2026-07-31 | Workbench payload architecture incident | Profiled the 124 MB dogfood database after the renderer became barely interactive. The trace view eagerly retained hidden log and metric probes: metrics transferred 81.5 MB in ~4.9 s, logs 5.9 MB in ~2.4 s, and post-GC renderer heap reached ~667 MB. Protocol `0.4.0` adds grouped service facets; inactive views neither fetch nor subscribe; live queries are signal-scoped, throttled, and coalesced; metrics use one compound indexed tail query plus a one-point instrument list / selected 120-point history split; logs are capped at 100 rows. Re-profiled startup at ~3.7 MB heap, trace list IPC at ~7 ms, metric list at ~36 ms / 83 KB, selected history at ~5 ms, and visible tab/selection updates at 7–26 ms. |
...
| 2026-07-14 | Packaged smoke test | Added `scripts/smoke.mjs` (npm `smoke`), which launches the packed Linux binary and asserts the release-critical runtime path: the OTLP receiver answers `/healthz`, the sandboxed preload bridge loads, the Traces workbench renders, closing the renderer leaves the tray runtime receiving, a valid OTLP/HTTP JSON trace ingests (`200` while hidden), request hardening survives packaging (non-JSON `POST` → `415`), then explicit process shutdown removes runtime ownership. It runs Electron in its own process group and escalates SIGTERM→SIGKILL so it always terminates, uses random OTLP/debugging ports and a temp user-data dir, and exits non-zero on any failed assertion. Added `package:dir` to produce the unpacked app for local smoke runs. The release workflow invokes `package` and then smokes its unpacked output under `xvfb`. |
