...
- Desktop main/preload integration tests cover settings validation and migration, revision/CAS conflicts, port rebinding, rollback after bind or persistence failure, IPC request and method-specific result validation, lifecycle cleanup, and the exposed context-bridge surface.
...
- CLI tests enforce lifecycle/status/endpoints/doctor behavior, JSON output, exit codes, unknown-command rejection, and shared-owner shutdown waits; source smoke checks a live Desktop-started daemon without opening SQLite.
...
- Performance checks use checked-in deterministic workloads and enforce [spec.md](spec.md) budgets. They cover exact trace/span ID and span attribute key/value searches over 10,000 trace summaries / 200,000 stored spans, 10,000-wide and 5,000-deep traces, 50 rapid selections, continuous ingest, query/IPC counts and bytes, React commits/row rerenders, mounted DOM, stack safety, renderer heap, cache/queue caps, and packaged pointer/scroll latency. Durable query tests separately enforce executed-statement budgets, one transaction per ingest/clear mutation, and representative `EXPLAIN QUERY PLAN` index use; substring log search remains an expected scan until FTS5. Traces startup issues no hidden log/metric lists; facets stay below 16 KiB; metric lists carry bounded latest-value summaries without point/resource/scope bags, and only the selected metric loads event-time-ordered cursor pages with explicit attribute-truncation metadata. Effects are used only for external synchronization, and tests reject stale waterfall commits or root-wide selection rerenders.
...
- **macOS expected**: attach the architecture-matching DMG and extract the matching ZIP into isolated Applications directories, run the packaged smoke against each copied bundle, remove them, and confirm both are gone.
...
- The artifact smoke requires bundled `oteluxctl` to own start → status → restart to a different exact instance → stop and remove matching ownership in unpacked, extracted `.deb`, and extracted AppImage layouts.
- The competing-start migration regression proves Desktop passes its legacy source to the daemon, the daemon claims ownership before any migration copy/resume, and a losing starter performs no migration writes. Release-resolution coverage and every native package command require Desktop package, lockfile, bundled CLI package, and CLI lockfile versions to match.
