...
- Before M2, extend the artifact smoke so bundled `oteluxctl` owns start → status → restart to a different exact instance → stop and removes matching ownership in unpacked, extracted `.deb`, and extracted AppImage layouts.
- The competing-start migration regression proves Desktop passes its legacy source to the daemon, the daemon claims ownership before any migration copy/resume, and a losing starter performs no migration writes. Release-resolution coverage requires Desktop package, lockfile, bundled CLI package, and CLI lockfile versions to match.
