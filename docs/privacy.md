...
With the default **Keep receiver running in the background** setting, closing the desktop window or choosing Quit from the Dock hides the workbench while the menu-bar/tray control and on-demand runtime keep receiving telemetry. Turn the setting off if either action should also stop ingestion. Before deleting database files, choose **Quit OTelux and Stop Receiver** from the menu bar/tray and wait for `runtime.json` and `runtime.lock` to disappear. The action uses authenticated Runtime RPC; it does not signal an unverified PID.
