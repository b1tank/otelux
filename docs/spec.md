...
- `apps/desktop`: Electron shell hosting IPC/windows and the React renderer; its main process discovers or starts the on-demand daemon and proxies the existing IPC surface over Runtime HTTP/SSE.
...
- The storage audit's span-identity P0 is fixed in schema v2; trace-service count/page correctness is fixed in schema v3; schema v4 indexes the standard `service.namespace` source dimension; schema v5 indexes event-time metric history; metric histories are bounded and cursor-paged; protocol 0.6 provides grouped facets, lightweight waterfall payloads, cursor paging, and optional exact counts; and SQLite runs in a bounded worker. Executed-statement budgets and representative `EXPLAIN QUERY PLAN` index requirements are enforced for every current durable query family. See [storage.md](storage.md#audit-findings).
- `@otelux/protocol` now enforces bounded path-aware Electron and Runtime RPC/SSE validation, aggregate wire-string budgets, shared `runtime.json` decoding, tagged-bigint JSON wire codecs, compatibility fixtures, protocol-major negotiation, and checked draft 2020-12 schemas. A canonical exhaustive result registry validates every advertised telemetry/control method over HTTP and Electron IPC; shared parity fixtures and method-result schemas prevent transport drift. Direct dispatcher and authenticated HTTP/SSE tests are live. Metric discovery returns lightweight metadata/latest-value summaries and selected history loads separately through an opaque instrument ID with event-time ordering, 1,000-point pages, continuation cursors, explicit chart-attribute truncation metadata, and stale-reference recovery. Browser session/bootstrap remains; Desktop daemon-client conversion is live. See [protocol.md](protocol.md#current-gaps).
...
| `@otelux/agent-integrations` | Node-only typed detection, inspection, planning, mutation, and verification for coding-agent integrations. | M2 foundation: bounded contracts, safe host-inspection primitives, and read-only Claude Code 2.x inspection live; CLI exposure, other adapters, and mutation pending. |
...
|---|---|---|
| `apps/desktop` | Main Electron workbench. | `v0.1.17` is the latest published Linux x64/arm64 `.deb` and AppImage prerelease; unsigned macOS DMG/ZIP artifacts are preview targets and Windows remains deferred. |
...
|---|---:|---:|---:|---|
| Shared local runtime | `4318` | `4320` | `4321` | Avoids the standard collector on `4318`. OTLP/MCP are configurable and MCP can be disabled; Runtime API status is discovered through owner-only state. Runtime ownership is still embedded in Desktop. |
...
- Search uses one shared `SearchField` interaction and clear affordance across traces, logs, metrics, and detail drawers. Trace search covers trace/span IDs, names, services, and span attribute keys/values; log search covers IDs, body, attributes, resource, and scope; metric search covers identity, metadata, resource/scope, and point attributes. Detail searches open matching sections without changing selection.
- Live refresh preserves the mounted waterfall and detail drawer for the selected identity; only an explicit selection change may replace them with a loading state.
- Property sections expose counts before expansion.
- Rows expose actions consistently: details, copy, value viewer, and pivots. Clipboard actions use a sandbox-compatible fallback and show Copied only after a successful write.
