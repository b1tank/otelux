...
- Dense trace modes need polish; span and log detail drawers provide internal key/value search. Trace interaction now uses iterative layout, constant-DOM indentation, virtualized trace/waterfall rows, stable memoized row props, latest-only same-turn selection, stale-result rejection, and a bounded recent-trace cache. Remaining release risks are full-summary trace IPC payloads and synchronous SQLite execution in Electron main; the next protocol/runtime sprint owns those boundaries.
