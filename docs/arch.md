...
Today, an on-demand `oteluxd` process owns SQLite, retention, engine queries, OTLP, MCP, settings, sample data, lifecycle events, canonical data migration, and runtime ownership/state files. Electron main discovers or starts that daemon, authenticates over Runtime HTTP/SSE, and proxies its existing narrow renderer IPC contract. `otel_open_dashboard` still launches or focuses Electron, but ingest and MCP continue after Desktop exits.
