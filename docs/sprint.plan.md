...
The highest-impact interaction and backend defects are fixed: layout is iterative, indentation DOM is constant, trace and waterfall rows are virtualized, rapid same-turn selection launches one request, recent traces are bounded and cached, stale traces never present as the new selection, waterfall payloads omit full bags, selected details load separately, and SQLite runs in a bounded prioritized worker. OTLP concurrency is bounded with explicit overload responses and visible per-signal counters; trace/log keyset cursors and optional exact counts are live. Trace and log views expose cursor-backed `Load more` controls; appended pages skip exact counts while the initial page keeps the authoritative total. Release CI now runs a packaged performance gate over 10,000 traces / 200,000 spans plus deep/wide adversarial traces and concurrent ingest. Local evidence: 24 mounted trace rows, 42 waterfall rows / 410 waterfall DOM nodes, ~55 ms explicit cursor paging, 5–9 ms 50-click dispatch, 16.8–20.1 ms p95 frame gaps, and 5.3–5.8 MB post-GC renderer heap. Explicit paging is comfortably below the 500 ms threshold, so speculative near-end prefetch is deferred.
