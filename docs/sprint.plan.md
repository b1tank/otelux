...
- [x] **P1 — Add backpressure; disposition keyset pagination next.** Concurrent OTLP exports are capped at 64 and excess requests return explicit `503 receiver_overloaded`; the SQLite worker is independently capped at 512. Keyset cursor semantics and UI pressure counters remain in `docs/plan.md` as the next bounded task.
