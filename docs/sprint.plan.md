...
- [x] **P1 — Add backpressure and trace keyset pagination.** Concurrent OTLP exports are capped at 64 and excess requests return explicit `503 receiver_overloaded`; per-signal rejection counts propagate through runtime status to a `Dropped N` endpoint pill. The SQLite worker is independently capped at 512. Trace list results expose `nextCursor`, and memory/SQLite apply stable sort-field + trace-ID keysets; log cursors and optional exact counts remain in `docs/plan.md`.
